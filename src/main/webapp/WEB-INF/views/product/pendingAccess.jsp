...
window.onload = function(){
    ...
    xhr.open("GET","<c:url value='/findAllProductPendingAccess' />",true);
    xhr.send();
    xhr.onreadystatechange = function(){
...
function showData(textObj) {
    ...
    let segment = "<table border='1' style = 'width:1300px;text-align: center;'>";
        if (size == 0) {
			...
			segment += "<tr><th style='text-align: center;'>課程圖片</th><th style='text-align: center;'>課程名稱</th><th>課程類別</th><th>課程價格</th><th style='text-align: center;'>課程介紹</th><th width:50px; style='text-align: center;'>審核結果</th></tr>";
			for (n = 0; n < products.length; n++) {
				let product = products[n];
    			let tmp0 = "<c:url value = '/accessResult/'/>"+ product.p_ID;
				...
				segment += "<td><input type='button'value='通過' style='margin: 5px;' onclick=\"window.location.href='"+tmp0+"'\"'/>";
