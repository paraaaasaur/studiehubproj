...
<script>
	...
	function showData(textobj) {
		...
		if (size == 0) {
			...
			segment += "<tr><th>會員帳號</th><th>活動類型</th><th>活動名稱</th><th>活動開始時間</th><th>活動結束時間</th><th>活動地址</th><th>活動照片</th></tr>"
...
// 				console.log("<td><input type='button'value='刪除'onclick=if(confirm('是否確定刪除("+ event.a_name+ ")'))location='<c:url value = '/updateProduct/"+event.a_aid+"'/>' /></td>")
				...
				segment += "<td><input type='button'value='刪除'onclick=if(confirm('是否確定刪除("+ event.a_name + ")'))location='<c:url value = '/deleteadminEvent/"+event.a_aid+"'/>' /></td>"
